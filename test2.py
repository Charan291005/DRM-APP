...
def get_mac():
    try:
        ...
        return "00:00:00:00:00:00"  # Default MAC if unable to fetch
...
def log_action(action, filename, identifier, expiry):
    ...
    log_file_path = os.path.join(application_path, "log.csv")
...
class Encryptor:
    ...
    def create_widgets(self):
        ...
        self.pref_combo.grid(row=7, column=0, sticky="ew", pady=(0, 24), ipady=8)

        identifier_label = ttk.Label(container, text="Enter Identifier (leave blank for current based on preference):",
                                      wraplength=700)
        identifier_label.grid(row=8, column=0, sticky="w", pady=(0, 8))

        self.identifier_entry = ttk.Entry(container, font=("Inter", 16))
        self.identifier_entry.grid(row=9, column=0, sticky="ew", pady=(0, 24), ipady=10)

        password_label = ttk.Label(container, text="Enter password (required):", wraplength=700)
        password_label.grid(row=10, column=0, sticky="w", pady=(0, 12))

        self.password_entry = ttk.Entry(container, show="*", font=("Inter", 16))
        self.password_entry.grid(row=11, column=0, sticky="ew", pady=(0, 40), ipady=10)

        self.encrypt_button = ttk.Button(container, text="Encrypt File", command=self.encrypt_file)
        self.encrypt_button.grid(row=12, column=0, pady=0, sticky="ew", ipadx=20, ipady=14)
    ...
    def encrypt_file(self):
        ...
        pref = self.pref_combo.get()
        identifier = self.identifier_entry.get().strip()

        if pref == "MAC Address":
            identifier = identifier or get_mac()
        elif pref == "IP Address":
            identifier = identifier or get_ip()
        else:  # pref == "None"
            identifier = "None"
        ...
        try:
            encrypted_file, _ = encrypt_file_util(file_path, expiry, identifier, password)
            log_action("ENCRYPT", os.path.basename(encrypted_file), identifier, expiry)
...
class Viewer:
    ...
    def decrypt_and_view(self, file_path, password):
        try:
            ...
            try:
                expiry_str, identifier, ext, file_password = header_line.split("|")
            except Exception:
            ...
            if datetime.now() > expiry_dt:
                ...
                log_action("VIEW_DENIED_EXPIRED", os.path.basename(file_path), identifier, expiry_str)
                return

            # No identifier check if identifier is "None"
            if identifier != "None":
                current_mac = get_mac()
                current_ip = get_ip()
                if identifier.lower() != current_mac.lower() and identifier.lower() != current_ip.lower():
                    messagebox.showerror("Access Denied", "Identifier mismatch. Access denied.")
                    log_action("VIEW_DENIED_IDENTIFIER", os.path.basename(file_path), identifier, expiry_str)
                    return

            if file_password != password:
                messagebox.showerror("Access Denied", "Incorrect password.")
                log_action("VIEW_DENIED_PASSWORD", os.path.basename(file_path), identifier, expiry_str)
                return

            key = hashlib.sha256((identifier + expiry_str + password).encode()).digest()
            cipher = AES.new(key, AES.MODE_CBC, iv)
            ...
            log_action("VIEW_SUCCESS", os.path.basename(file_path), identifier, expiry_str)
